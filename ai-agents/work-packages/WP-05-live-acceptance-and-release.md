...
Process complete Paper/Leaf acceptance evidence for the WP-04 release candidate, fix every confirmed defect in the same fixed package, rerun affected and regression cases, and publish the production EnthusiaLoreItems release.

## Owner-approved scope amendment — 2026-08-07

The project owner explicitly removed real Microsoft/Xbox account authentication from WP-05 acceptance. WP-05 must **not** require or solicit Microsoft credentials, device-code sign-ins, Java entitlement checks, or Xbox/Bedrock account authentication.

Identity-sensitive coverage remains in scope, but it validates the behavior LoreItems can actually observe at the server boundary:

- Java server-visible UUID/name handling;
- Floodgate/Geyser `*`-prefixed Bedrock server-visible identity handling;
- cached-offline and never-joined recipient resolution where applicable;
- command, GUI, delivery, audit, and distribution behavior attached to those server-visible identities.

Disposable protocol clients, test identities, and deterministic server-side harnesses are acceptable for those cases. Evidence must state the authentication mode used and must not claim Microsoft/Xbox authentication coverage when none occurred. Any older matrix prerequisite requiring a real authenticated Microsoft/Xbox account is superseded by this scope amendment. This is a requirements/planning change, not a claim that an unexecuted case passed.

## Dependencies

- WP-04 is `COMPLETE`.
- GitHub prerelease `v1.0.0-rc.1` and its checksum/artifacts point to the verified WP-04 merge commit.
- A designated acceptance environment can run Java 21 and the target Paper/Leaf 1.21.11-compatible build with Geyser and Floodgate. A disposable GitHub Actions environment is acceptable when it faithfully exercises the server-visible behavior required by the case.

## Complete required scope

1. Execute every in-scope case ID in the WP-04 manual checklist against the exact RC jar. Evidence for each case must record server implementation/build, Java version, plugin version, jar SHA-256, schema version, configuration, test identities, authentication mode, timestamps, precise steps, expected result, actual result, relevant logs/database queries/metrics, cleanup, and PASS/FAIL/BLOCKED status.
2. Cover at minimum:
   - Java and `*`-prefixed Bedrock server-visible identity and commands/GUI, without requiring Microsoft/Xbox account authentication;
...
7. Repeat the complete **in-scope** matrix on the final release-candidate build after the last code change. All in-scope cases must be PASS. The account-authentication prerequisite removed by the owner-approved amendment is not an acceptance case and must not be represented as PASS. Other `BLOCKED`, waived, not-run, and unsupported claims do not approve release.
...
- Every required in-scope acceptance case has complete GitHub-backed evidence for the exact tested jar SHA and final result PASS.
- Identity cases prove Java and Floodgate/Geyser server-visible behavior; they do not require or claim Microsoft/Xbox account authentication.
...
- Independent review of all code fixes and a separate evidence audit confirming every in-scope case ID, jar SHA, expected/actual result, and permanent evidence reference.
- Release review explicitly covers item loss, duplicate creation, wrong-target destruction, main-thread work, bounds, reload/shutdown, migration/rollback, Geyser/Floodgate server-visible identity behavior, API, and unsupported claims.
- No requested changes, zero unresolved threads, all exact-head checks successful.
- Owner/operator sign-off recorded on GitHub after the final in-scope matrix.
- Normal merge commit, verified main checks, and verified `v1.0.0` tag/release assets.

## Explicit exclusions

- Real Microsoft/Xbox credential collection, device-code authentication, entitlement verification, or claims that such authentication was tested.
- Waiving or deferring a confirmed implementation defect.
- Treating an unexecuted, blocked, or screenshot-only in-scope case as PASS.
...
WP-05 is complete only when the final full **in-scope** acceptance matrix is all PASS, every confirmed defect is fixed and retested, all automated/review gates pass, the PR is normally merged, live `main` is verified, `v1.0.0` is published from that merge with correct artifacts, durable state is updated, and the worker stops.

## Expected status transitions

`READY -> IN_PROGRESS -> EVIDENCE_PENDING -> IN_PROGRESS (for any defect) -> IN_REVIEW -> VERIFYING -> MERGED -> COMPLETE`

Any failed in-scope case returns to `IN_PROGRESS` within WP-05. The package remains `MERGED` until the production release is verified.
