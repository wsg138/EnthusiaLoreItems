        ...
        return storage.execute(connection -> {
            try (PreparedStatement statement = connection.prepareStatement(
                    "SELECT d.definition_id, d.lookup_key, d.display_name, d.current_revision, "
                            ...
                            + "(SELECT COUNT(*) FROM lore_instances pending WHERE "
                            + "pending.definition_id = d.definition_id "
                            + "AND pending.lifecycle_state = 'ACTIVE' "
                            + "AND pending.applied_revision < d.current_revision) pending_count "
